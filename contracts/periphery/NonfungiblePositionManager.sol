    ...
    ERC721Enumerable
{
    ...
    modifier isAuthorizedForToken(uint256 tokenId) {
        ...
        require(_isAuthorized(tokenOwner, msg.sender, tokenId));
        _;
    }
        ...
        returns (uint256 amount0, uint256 amount1)
    {
        ...
        if (position.liquidity > 0) {
            ...
            if (!isStaked) {
                ...
                (, feeGrowthInside0LastX128, feeGrowthInside1LastX128, , ) = pool.positions(
                    PositionKey.compute(gauge, position.tickLower, position.tickUpper)
                );
            }
    ...
    /// @inheritdoc ERC721
    function _getApproved(uint256 tokenId) internal view override(ERC721) returns (address) {
