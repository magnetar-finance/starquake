...
contract CLGauge is ICLGauge, ERC721Holder, ReentrancyGuard {
    ...
    error NoStakes(address, uint256);
    error OnlyVoter();

    modifier onlyVoter() {
        if (msg.sender != address(voter)) revert OnlyVoter();
        _;
    }
    ...
    function initialize(
        ...
        require(address(pool) == address(0), 'PI');
    ...
    function earned(address account, uint256 tokenId) external view override returns (uint256) {
        if (!_stakes[account].contains(tokenId)) revert NoStakes(account, tokenId);
    ...
    function getReward(uint256 tokenId) external override nonReentrant {
        if (!_stakes[msg.sender].contains(tokenId)) revert NoStakes(msg.sender, tokenId);
    ...
    function _notifyRewardAmount(address _sender, uint256 _amount) internal {
        ...
        if (timestamp >= periodFinish) {
            ...
            pool.syncReward({
                ...
                rewardReserve: _amount + _leftover,
    ...
    function _claimFees() internal {
        ...
        if (claimed0 > 0 || claimed1 > 0) {
            ...
            if (_fees0 > ReactorTimeLibrary.WEEK) {
                fees0 = 0;
            ...
            if (_fees1 > ReactorTimeLibrary.WEEK) {
                fees1 = 0;
