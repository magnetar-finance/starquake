...
contract E2E_mint_burn {
    ...
    //
    //
    function _getRandomPositionIdx(uint128 _seed, uint256 _positionsCount) internal pure returns (uint128 positionIdx) {
    ...
    function testEchidna_burn_partial(uint128 _amount) public {
        ...
        try minter.doBurn(pos.tickLower, pos.tickUpper, burnAmount) returns (
            CLMinter.MinterStats memory bfre_burn,
            CLMinter.MinterStats memory aftr_burn
        ) {
            bfre = bfre_burn;
            aftr = aftr_burn;
        } catch {
