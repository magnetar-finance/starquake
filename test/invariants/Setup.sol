...
contract CLMinter is ERC721Holder {
    ...
    function deposit(uint256 tokenId) public returns (StakingData memory sd) {
        ...
        (, , , , , , , , sd.feeGrowthInside0LastX128Before, sd.feeGrowthInside1LastX128Before, , ) = nft.positions(
            tokenId
        );
        nft.approve(address(gauge), tokenId);
        gauge.deposit(tokenId);
        ...
        (
            ...
            ,
            ,
            sd.feeGrowthInside0LastX128After,
            sd.feeGrowthInside1LastX128After,
            sd.tokensOwed0,
            sd.tokensOwed1
