...
contract RewardGrowthGlobalFuzzTest is CLPoolTest {
    ...
    function mintNewFullRangePositionAndDepositIntoGauge(
        uint128 _amount0,
        uint128 _amount1,
        address _user
    ...
    function testFuzz_RewardsRolledOverToNextEpochIfThereIsNoStakedLiquidityAtRewardDistribution(
        ...
        assertApproxEqAbs(
            aliceRewardBalance,
            rewardRate * (WEEK - (time + delay)) + (reward + rewardRate * time) / 2,
            1e6
        );
        ...
        assertApproxEqAbs(
            ...
            aliceRewardBalance + (reward + rewardRate * time) / 2,
            1e6
        );
    ...
    function testFuzz_RewardGrowthGlobalUpdatesCorrectlyWithUnstakedLiquidityBothAtTheStartAndEndOfTheEpoch(
        ...
        assertApproxEqAbs(
            pool.rewardRate(),
            (reward2 + reward - (rewardRate * (WEEK - (time + time2 + delay)))) / (WEEK - 200),
            1e4
        );
    ...
    function testFuzz_RewardGrowthGlobalUpdatesCorrectlyWithLateNotifyAndUnstakedLiquidityCrossingEpochFlips(
        ...
        assertApproxEqAbs(
            rewardToken.balanceOf(users.alice) - accumulatedReward,
            rewardRate * (WEEK - (time + time2)),
            1e7
        );
    ...
    function testFuzz_RewardGrowthEpochSkipped(
        ...
        assertApproxEqAbs(
            gauge.rewardRate(),
            (reward * 2 + rewardRate * (WEEK - (time + delay))) / (WEEK - time2),
            1e6
        );
